...
int32_t Sockbuf_flush(sockbuf_t *sbuf)
{
    ...
    if (BIT(sbuf->state, SOCKBUF_DGRAM) != 0)
    {
        ...
        if (len != sbuf->len)
        {
            warn("Can't write complete datagram (%d,%d)", len,
                 sbuf->len);
        }
        Sockbuf_clear(sbuf);
    }
...
int32_t Sockbuf_read(sockbuf_t *sbuf)
{
    ...
    {
        warn("No read from non-readable socket buffer (%d)",
             sbuf->state);
        return -1;
    }
    if (BIT(sbuf->state, SOCKBUF_LOCK) != 0)
    ...
    else
    {
        ...
        /*	Trace("Read stream %d bytes from %d\n", len, sbuf->sock); */
        sbuf->len += len;
    }
            ...
            case 's': /* Small strings */
                ...
                for (;;)
                {
                    ...
                    else if (k >= max_str_size)
                    {
                         ...
                         */
                        warn("String overflow while scanning (%d,%d)", k, max_str_size);
                        if (BIT(sbuf->state, SOCKBUF_LOCK) != 0)
                        {
                            failure = 2;
                        }

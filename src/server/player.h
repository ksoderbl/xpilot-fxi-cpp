 ...
 */
constexpr int32_t PL_TYPE_HUMAN = 1U << 0;
constexpr int32_t PL_TYPE_ROBOT = 1U << 1;
constexpr int32_t PL_TYPE_TANK = 1U << 2;

/*
 * These values are set in the player->pl_state field.
 */
constexpr int32_t PL_STATE_UNDEFINED = 1U << 0;
constexpr int32_t PL_STATE_WAITING = 1U << 1;
constexpr int32_t PL_STATE_APPEARING = 1U << 2;
constexpr int32_t PL_STATE_ALIVE = 1U << 3;
constexpr int32_t PL_STATE_KILLED = 1U << 4; /* temporary state; should transform to APPEARING, DEAD or PAUSED in the same frame */
constexpr int32_t PL_STATE_DEAD = 1U << 5;
constexpr int32_t PL_STATE_PAUSED = 1U << 6;
...
typedef struct player
{
    ...
    struct base *home_base; /* Pointer to the home base */
    ...
    struct xp_object *ball_tmp; /* Ball that is BEING attached (string not solid yet) */
    ...
    struct connection *connp; /* pointer to the connection structure */
...
void Player_change_home(player_t *pl, struct base *base);
player_t *Player_owns_base(struct base *base);
...
struct xp_object *Player_get_ball_object(player_t *pl);
